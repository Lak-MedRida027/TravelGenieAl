...
class Config:
    ...
    # ADK Configuration
    ADK_AGENT_ID = os.getenv("ADK_AGENT_ID")
    ADK_PROJECT_ID = os.getenv("ADK_PROJECT_ID")
    ADK_LOCATION = os.getenv("ADK_LOCATION", "global")
    ADK_CREDENTIALS = os.getenv("ADK_CREDENTIALS")

app = FastAPI(
    title="AI Travel Agent API with ADK Integration",
    description="Advanced travel planning API with Google ADK conversational capabilities",
    version="2.0"
)
...
class TravelPlanResponse(BaseModel):
    ...
    status: str
    adk_enhancement: Optional[Dict[str, Any]] = None
...
class ADKAgentService:
    def __init__(self):
        if not all([Config.ADK_AGENT_ID, Config.ADK_PROJECT_ID, Config.ADK_CREDENTIALS]):
            logger.warning("ADK configuration incomplete - ADK features will be disabled")
            self.enabled = False
            return
            
        try:
            self.credentials = service_account.Credentials.from_service_account_file(
                Config.ADK_CREDENTIALS
            )
            self.client = AgentsClient(credentials=self.credentials)
            self.agent_path = self.client.agent_path(
                Config.ADK_PROJECT_ID,
                Config.ADK_LOCATION,
                Config.ADK_AGENT_ID
            )
            self.enabled = True
        except Exception as e:
            logger.error(f"Failed to initialize ADK service: {str(e)}")
            self.enabled = False
            
    async def initialize_session(self, session_id: str) -> Optional[str]:
        """Initialize a session with the ADK agent"""
        if not self.enabled:
            return None
        ...
        try:
            ...
            logger.error(f"Failed to initialize ADK session: {str(e)}")
            return None
            
    async def detect_intent(self, session_path: str, text: str) -> ADKResponse:
        """Send a text query to the ADK agent and get the response"""
        if not self.enabled:
            return ADKResponse(
                response_text="ADK service is not configured",
                intent=None,
                confidence=0,
                parameters={}
            )
        ...
        try:
            ...
            logger.error(f"ADK intent detection failed: {str(e)}")
            return ADKResponse(
            ...
            logger.error(f"Unexpected error in ADK intent detection: {str(e)}")
            return ADKResponse(
...
class ItineraryService:
    ...
    async def generate_full_plan(self, flights: List, hotels: List, request: TravelRequest, session_id: Optional[str] = None) -> Dict:
        prompt = self._build_prompt(flights, hotels, request)
        
        try:
            response = self.model.generate_content(prompt)
            plan = self._parse_response(response.text, flights, hotels, request)
            
            # Enhance with ADK if available
            if session_id and self.adk_service.enabled:
                session_path = await self.adk_service.initialize_session(session_id)
                if session_path:
                    summary = (
                        f"Travel plan generated for {request.travelers} people from {request.source} "
                        f"to {request.destination} from {request.departure_date} to {request.return_date}. "
                        f"Budget: {request.budget}. Interests: {', '.join(request.interests)}. "
                        f"Please provide any additional recommendations or improvements."
                    )
                    adk_response = await self.adk_service.detect_intent(session_path, summary)
                    plan["adk_enhancement"] = {
                        "suggestions": adk_response.response_text,
                        "related_intents": adk_response.intent,
                        "confidence": adk_response.confidence,
                        "extracted_parameters": adk_response.parameters
                    }
...
async def generate_travel_plan(request: TravelRequest, session_id: Optional[str] = None):
    """
    ...
    - Detailed itinerary
    - Budget breakdown
    - Optional ADK enhancement if session_id provided
    """
    try:
        ...
        plan = await itinerary_service.generate_full_plan(flights, hotels, request, session_id)
...
@app.post("/adk/initialize-session")
async def initialize_adk_session():
    """
    Initialize a new session with the ADK agent
    Returns session ID for subsequent requests
    """
    try:
        session_id = str(datetime.now().timestamp())
        adk_service = ADKAgentService()
        session_path = await adk_service.initialize_session(session_id)
        ...
        if session_path:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Failed to initialize ADK session"
            )
    except Exception as e:
        logger.error(f"Error initializing ADK session: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=str(e)
        )

@app.post("/adk/detect-intent", response_model=ADKResponse)
async def detect_intent(session_id: str, text: str):
    """
    Detect intent from user text using ADK agent
    Requires an active session_id from /adk/initialize-session
    """
    try:
        adk_service = ADKAgentService()
        session_path = f"{adk_service.agent_path}/sessions/{session_id}"
        response = await adk_service.detect_intent(session_path, text)
...
def format_travel_plan(plan: TravelPlanResponse) -> str:
    ...
    # Add ADK enhancements if available
    if hasattr(plan, 'adk_enhancement') and plan.adk_enhancement:
        text += "\n🤖 AI SUGGESTIONS:\n"
        text += f"  {plan.adk_enhancement['suggestions']}\n"
        if plan.adk_enhancement['related_intents']:
            text += f"  (Detected intent: {plan.adk_enhancement['related_intents']} with confidence {plan.adk_enhancement['confidence']:.2f})\n"
